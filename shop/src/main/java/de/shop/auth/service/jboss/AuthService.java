...
public class AuthService implements Serializable {
	...
	public enum RolleType {
		ADMIN(0), MITARBEITER(1), ABTEILUNGSLEITER(2), KUNDE(3);
	...
	/**
	 */
	public void addRollen(Integer kundeId, Collection<RolleType> rollen) {
		if (rollen == null || rollen.isEmpty()) {
			return;
		}

		ks.findKundeById(kundeId, null).getRollen().addAll(rollen);
		flushSecurityCache(kundeId.toString());
	}

	/**
	 */
	public void removeRollen(Integer kundeId, Collection<RolleType> rollen) {
		if (rollen == null || rollen.isEmpty()) {
			return;
		}

		ks.findKundeById(kundeId, null).getRollen().removeAll(rollen);
		flushSecurityCache(kundeId.toString());
	}
	 ...
	 * jboss-admin.bat connect
	 * /subsystem=security/security-domain=shop:flush-cache
	 * (principal=myUserName)
	 */
	private void flushSecurityCache(String username) {
		ModelControllerClient client;
		try {
			client = ModelControllerClient.Factory.create(LOCALHOST,
					MANAGEMENT_PORT);
		...
		try {
			...
			if (client != null) {
				try {
					...
					throw new IllegalStateException(
							"FEHLER bei der Methode close() fuer den Management-Client",
							e);
				}
			}
		}
	 ...
	 * em.createNamedQuery(KUNDE_BY_ID, String.class)
	 * .setParameter(PARAM_USERNAME_PREFIX, usernamePrefix + '%')
	 * .getResultList(); return usernameList; }
	 */
